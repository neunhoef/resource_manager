...
// The class BoundedList2 implements a high-performance bounded list using
// ResourceManager for the current AtomicList and implements the rotation logic similar to BoundedList.
// One can only prepend items and this operation is normally very fast.
// Every once in a while a prepend operation can be slightly slower if a batch
// of old items has to be freed. One can give an upper limit for the memory
// usage in the "current" list (argument _memoryThreshold) and once this limit
// is reached, a new current list is started and the old one is moved to a
...
class BoundedList2 {
 private:
  ...
  // Mutex for protecting the ring buffer
 ...
 public:
  ...
  BoundedList2(std::size_t memoryThreshold, std::size_t maxHistory)
    ...
    if (memoryThreshold == 0 || maxHistory < 2) {
      throw std::invalid_argument(
          "BoundedList2: memoryThreshold must be > 0 and maxHistory must be >= "
          "2");
    }
  }
  ...
  void prepend(T&& value) {
    ...
    _resourceManager.read([&value](const std::shared_ptr<AtomicList<T>>& current) {
  ...
  // Rotate lists when memory threshold is reached
  void rotateLists() {
    ...
    // Update the ring buffer under mutex protection
    {
      ...
      // No need to explicitly delete the old list as it will be automatically
      // reclaimed when no longer referenced
    }
  ...
  void forItems(F&& callback) const {
    ...
    std::vector<std::shared_ptr<AtomicList<T>>> snapshots;
    {
      ...
      _resourceManager.read([&snapshots](const std::shared_ptr<AtomicList<T>>& current) {
