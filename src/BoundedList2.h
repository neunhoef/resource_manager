...
// ResourceManager for the current AtomicList and implements the rotation logic
// similar to BoundedList. One can only prepend items and this operation is
// normally very fast. Every once in a while a prepend operation can be slightly
// slower if a batch of old items has to be freed. One can give an upper limit
// for the memory usage in the "current" list (argument _memoryThreshold) and
// once this limit is reached, a new current list is started and the old one is
// moved to a ring buffer of historic lists. The length of this ring buffer can
// be configured with the _maxHistory argument. The total upper limit for the
...
template <typename T> class BoundedList2 {
private:
...
public:
  ...
  BoundedList2(std::size_t memoryThreshold, std::size_t maxHistory)
      : _resourceManager(std::make_unique<std::shared_ptr<AtomicList<T>>>(
        ...
        _memoryUsage(0), _history(maxHistory), _ringBufferPos(0),
